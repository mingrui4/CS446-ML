...
def train(model, mnist_dataset, learning_rate=0.0005, batch_size=16,
    ...
    for step in range(0, num_steps):
        ...
        #         )
        model.session.run(model.update_op_tensor_d, feed_dict={model.x_placeholder: batch_x, model.z_placeholder: batch_z,model.learning_rate_placeholder: learning_rate})
        model.session.run(model.update_op_tensor_g, feed_dict={model.z_placeholder: batch_z,model.learning_rate_placeholder: learning_rate})
...
def main(_):
    ...
    std = 1
    x_z = np.linspace(-1 * std, 1 * std, 20)
    y_z = np.linspace(-1 * std, 1 * std, 20)
