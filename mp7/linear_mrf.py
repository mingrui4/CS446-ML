...
class LinearMRF(object):
    ...
    def get_pairwise_features(self):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        f_pairwise = np.zeros((len(self.pairs),4))
    ...
    def calculate_unary_potentials(self, unary_features):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        phi_i = tf.multiply(self.unary_weight,unary_features)
        return phi_i
    ...
    def calculate_pairwise_potentials(self, pairwise_features):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        phi_ij = tf.multiply(self.pairwise_weight, pairwise_features)
        return phi_ij
    ...
    def build_training_obj(self, img_features, unary_beliefs, pair_beliefs,
        ...
        for i in range(len(unary_beliefs)):
            x1 = tf.reduce_sum(tf.multiply(unary_beliefs[i],unary_potentials[i]))
            x2 = tf.reduce_sum(tf.multiply(pair_beliefs[i],pairwise_potentials))
            f1 = self.calculate_unary_potentials(img_features)
            F = tf.reduce_sum(img_features*unary_potentials[i]) + tf.reduce_sum(self.get_pairwise_beliefs(img_features)*pairwise_potentials)
            #F = tf.reduce_sum(img_features*f1[i]) + tf.reduce_sum(self.get_pairwise_beliefs(img_features)*pairwise_potentials)
            obj += x1 + x2 - F
        return obj
    ...
    def inference_itr(self, unary_beliefs, unary_pots, pairwise_pots):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        index = list(range(0,len(unary_beliefs)))
        np.random.shuffle(index)
        for i in range(len(unary_beliefs)):
            node = index[i]
            assignment_0 = 0
            score_0 = self.calculate_local_score(node, assignment_0, unary_beliefs,
                                           unary_pots, pairwise_pots)
            assignment_1 = 1
            score_1 = self.calculate_local_score(node, assignment_1, unary_beliefs,
                                           unary_pots, pairwise_pots)
            if score_0 > score_1:
                unary_beliefs[node][assignment_0] = 1
                unary_beliefs[node][assignment_1] = 0
            else:
                unary_beliefs[node][assignment_0] = 0
                unary_beliefs[node][assignment_1] = 1
    ...
    def calculate_local_score(self, node, assignment, unary_beliefs,
        ...
        for pair_node in pairs_list:
            ...
            pairs_score+=pair_potential
        score = np.sum(unary_potentials[node][assignment] + pairs_score)
        return score
    ...
    def check_convergence(self, new_unary_beliefs, old_unary_beliefs,
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        # con = ( - new_unary_beliefs.shape[0] + old_unary_beliefs.shape[0])/old_unary_beliefs.shape[0]
        n = new_unary_beliefs.shape[0]
        for i in range(n):
            if new_unary_beliefs[i][0] == old_unary_beliefs[i][0] and new_unary_beliefs[i][1] == old_unary_beliefs[i][1]:
                num += 0
            else:
                num += 1
        con = num / n
        if con >= convergence_margin:
            converge = False
        else:
            converge = True
        return converge
    ...
    def get_pairwise_beliefs(self, unary_beliefs):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        for node,i in self.pair_inds.items():
            y1 = unary_beliefs[node[0]][1]
            y2 = unary_beliefs[node[1]][1]
            if y1 == 0 and y2 == 0:
                result[i][0] = 1
            elif y1 == 0 and y2 == 1:
                result[i][1] = 1
