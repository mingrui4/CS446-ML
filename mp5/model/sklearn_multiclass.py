...
def sklearn_multiclass_prediction(mode, X_train, y_train, X_test):
    ...
    if mode == 'ovr':
        ...
        clf = multiclass.OneVsOneClassifier(svm.LinearSVC(random_state = 111))
        clf.fit(X_train, y_train)
        y_pred_train = clf.predict(X_train)
        y_pred_test = clf.predict(X_test)
        return y_pred_train, y_pred_test
        ...
        libsvm = svm.LinearSVC(multi_class='crammer_singer', C=1)
        libsvm.fit(X_train, y_train)
        y_pred_train = libsvm.predict(X_train)
        y_pred_test = libsvm.predict(X_test)
        return y_pred_train, y_pred_test
        pass
