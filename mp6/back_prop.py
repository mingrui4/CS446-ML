...
class Neural_Network():
    ...
    def forward(self, X):
        ...
        '''
        z = X.dot(self.U) + self.e
        b = self.sigmoid(z)
        h = b.dot(self.W) + self.f
        c = self.sigmoid(h)
        return c
    ...
    def error_at_layer1(self, delta_k, W, b):
        ...
        '''
        delta_j = (delta_k * W.transpose()) * self.sigmoidPrime(b)
        return delta_j
    ...
    def derivative_of_w(self, b, delta_k):
        ...
        '''
        d_w = np.sum(delta_k * b.T,axis=1).reshape(hiddenSize, outputSize)
        return d_w
    ...
    def derivative_of_u(self, X, delta_j):
        ...
        '''
        d_u = np.dot(X.transpose(),delta_j)
        return d_u
